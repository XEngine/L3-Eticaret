		...
		<div class="row-fluid">
			<div class="span6">
							...
							@foreach($categories as $category)
								<optgroup label="{{$category->getDescriptions->name}}">
									@foreach($category->getChildren as $subcat)
										...
										?>
										<option {{$label}} value="{{$subcat->id}}">{{$subcat->getDescriptions->name}}</option>
					...
					<div class="controls">
						<select style="width:100%" name="p_brand" id="e1">
							@foreach($brands as $brand)
								...
								?>
								<option {{$label}} value="{{$brand->id}}">{{$brand->name}}</option>
							@endforeach
						</select>
				...
				<div class="control-group">
					...
					<div class="controls">
						<input style="width:100%" value="{{$product->model}}" id="p_model" name="p_model" type="text" placeholder="örn: DDL-220W">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label" for="inputEmail">Stok Adeti</label>
					<div class="controls">
						<input style="width:100%" value="{{$product->quantity}}" name="p_qty" type="text" placeholder="örn: 78">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label" for="inputEmail">Fiyat</label>
					<div class="controls">
						<input style="width:100%" value="{{$product->price}}" name="p_price" type="text" placeholder="örn: 7822,99">
			...
			<div class="span6">
				<div class="control-group">
					...
					<div class="controls">
						<input style="width:100%" value="{{$product->sku}}" name="p_sku" id="sku" type="text" placeholder="örn: SK1239334">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label" for="inputEmail">Barkod</label>
					<div class="controls">
						<input style="width:100%" value="{{$product->barcode}}" name="p_barcode" id="barcode" type="text" placeholder="örn: 8691234560013">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label" for="inputEmail">Ürün Kısa açıklaması</label>
					<div class="controls">
						<input style="width:100%" value="{{$product->getDetail->description}}" name="p_description" type="text" placeholder="örn: Bu ürün suda batmıyor!">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label" for="inputEmail">Detaylı açıklama</label>
					<div class="controls">
						<input style="width:100%" value="{{$product->getDetail->variant}}" name="p_variant"  type="text" placeholder="örn: Intel Core i7 3630QM 2.4GHz 16GB 750GB+256GB SSD 17.3'' 3D Taşınabilir Bilgisayar">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label"for="inputEmail">Tagler</label>
					<div class="controls">
						<input id="e20" value="{{$product->getDetail->tag}}" name="p_tag" type="text" style="width:100%">
					</div>
				</div>
			</div>
		</div>
			...
			<div class="span6">
				<div id="attributes">
					@for($x = 0;$x < count($attributes); $x++)
						<p class="lead">{{$attributes[$x]->name}}</p>
						@for($y = 0;$y < count($attributes[$x]->getParentGroup); $y++)
						<?php $itemA = $attributes[$x]->getParentGroup; ?>
						<?php die(print_r($itemA)); ?>
						<div class="control-group">
							<label class="control-label" for="inputEmail">{{$itemA->name}}</label>
							<div class="controls">
								<select name="attr[{{$itemA->id}}]" style="width:100%" id="attr_{{$itemA->id}}_'+i+'">
								@if(count($itemA) > 0){
									@foreach($itemA->getAttributes as $itemB)
										<option value="{{$itemB->id}}">{{$itemB->value}}</option>'
									@endforeach
								@else
									<option value=""></option>
								@endif
