...
class Checkout_Controller extends Base_Controller
{
    ...
    public function get_address()
    {
        ...
        if (isset($CartInfo['Addresses']) && $CartInfo['Addresses']['delivery'] != null) {
            $DefaultDelivery = Address::with(array(
                'getCity',
                'getTown'
            ))->where('id', '=', $CartInfo['Addresses']['delivery'])->first();
        } else {
            $DefaultDelivery = null;
        }
        if (isset($CartInfo['Addresses']) && $CartInfo['Addresses']['billing'] != null) {
            $DefaultBilling = Address::with(array(
                'getCity',
                'getTown'
            ))->where('id', '=', $CartInfo['Addresses']['billing'])->first();
        } else {
            $DefaultBilling = null;
        }
    ...
    public function get_payment()
    {
        ...
        $DeliveryAddress = Address::with(array(
            'getCity',
            'getTown'
        ))->where('id', '=', $Cart['Addresses']['delivery'])->first();
        $BillingAddress = Address::with(array(
            'getCity',
            'getTown'
        ))->where('id', '=', $Cart['Addresses']['billing'])->first();
